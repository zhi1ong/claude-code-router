...
export const appCopy: Record<ResolvedLanguage, AppCopy> = {
  en: {
    ...
    text: {
      ...
      "Default model list": "Default model list",
      "Default model list description": "When enabled, /v1/models returns the fixed Claude model list (Fable/Opus/Sonnet/Haiku) instead of the allowed model list, and requests for these models route through the profile model slots.",
      ...
      "Unidentified API key": "Unidentified API key",
      "group": "group",
      "groups": "groups",
  ...
  zh: {
    ...
    text: {
      ...
      "Client API key": "客户端 Key",
      "Unidentified API key": "未识别 Key",
      "group": "组",
      "groups": "组",
      ...
      "Linked Profile": "关联 Profile",
      "No linked Profile": "不关联 Profile",
      "Profile unavailable": "Profile 不可用",
      "Uses the Profile's routing and available models. The key stops working if the Profile is disabled or deleted.": "使用该 Profile 的路由和可用模型。Profile 被禁用或删除后，此密钥将不可用。",
      "Edit API key": "编辑 API 密钥",
      "Edit Profile": "编辑配置",
      "Edit Provider": "编辑供应商",
      "Edit provider": "编辑供应商",
      ...
      "Default model list": "返回默认模型列表",
      "Default model list description": "开启后 /v1/models 固定返回 Claude 四档模型列表（Fable/Opus/Sonnet/Haiku），忽略允许模型列表；请求这些模型时按 Profile 各档位选择路由。",
      "Restart proxy": "重启代理",
      "Route": "路由",
      "Route graph": "路由链路图",
      "Route hops": "路由节点",
